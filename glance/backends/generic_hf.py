"""The any-model backend: a Hugging Face image-text-to-text model with a chat template, read exactly as Qwen3-VL is read in
`glance/backends/vlm_hf.py`, but with nothing model-specific. Selected with `--model-id` (CLI), `Glance(model_id=...)` or
`models.generic` in the config. It began as the lab's second-family wrapper (E3, lab/NOTES.md entries 20, 22b, 44), where
SmolVLM2-2.2B is the one model it has been measured on; other models load, but their accuracy is NOT measured here.
...
class GenericVlmBackend:
    name = "vlm"
    kind = "vlm"

    def __init__(self, cfg: Config, model_id: str, revision: str | None = None, longest_edge: int | None = None, dtype: str = "auto",
        ...
        if dtype == "auto":  # half precision on an accelerator, full precision on the CPU
        ...
        if not self.revision:  # not pinned by the caller: record the commit that was actually loaded, so logs and fits stay tied to it
