...
index = []
for method, scale, meta in [(m, s, LADDERS[s]) for m in ("ens4d", "fast2") for s in LADDERS]:
    members = rating.MEMBERS[method]
    ...
    key = rating.RatingKey(backend="vlm", model=f"{spec.id}@{spec.revision}", prompt_version=rating.RATING_PROMPT_VERSION,
                           score_method=method, image_token_budget=spec.image_token_budget,
    ...
    cal = rating.build_calibration(key, x, y, name=f"lab/{scale}/{method}", rescale="train",
    ...
    index.append({"method": method, "name": cal.name, "version": cal.version, "file": path.name, "n": cal.n, "cv_accuracy": round(cal.cv["accuracy"], 3),
