...
def option_d():
    """The owner's design: cost (log) vertical, seconds (log) horizontal; per system three small marks (one per test) joined to
    one large mark at their centre (geometric mean, which is the visual centre on log axes); mean accuracy printed beside it."""
    x0, x1, y0, y1 = 46, 344, 20, 330
    sx, sy = (lambda v: log(v, 0.25, 6, x0, x1)), (lambda v: log(v, 0.04, 30, y1, y0))
    g = []
    for v in (0.3, 1, 3):
        g.append(f'<line x1="{sx(v):.1f}" x2="{sx(v):.1f}" y1="{y0}" y2="{y1}" class="grid"/>' + t(sx(v), y1 + 15, f"{v:g} s"))
    for v in (0.1, 1, 10):
        g.append(f'<line x1="{x0}" x2="{x1}" y1="{sy(v):.1f}" y2="{sy(v):.1f}" class="grid"/>' + t(x0 - 6, sy(v) + 4, money(v), "end"))
    shapes = {"yesno": lambda x, y, c: f'<circle cx="{x:.1f}" cy="{y:.1f}" r="3.2" class="{c}"/>',
              "choice": lambda x, y, c: f'<rect x="{x - 3:.1f}" y="{y - 3:.1f}" width="6" height="6" class="{c}"/>',
              "rating": lambda x, y, c: f'<polygon points="{x:.1f},{y - 3.8:.1f} {x + 3.6:.1f},{y + 2.8:.1f} {x - 3.6:.1f},{y + 2.8:.1f}" class="{c}"/>'}
    for name in m["systems"]:
        mine = [r for r in rows if r["name"] == name]
        gx = math.exp(statistics.mean(math.log(r["sec"]) for r in mine))
        gy = math.exp(statistics.mean(math.log(r["usd"]) for r in mine))
        acc = statistics.mean(r["acc"] for r in mine)
        own = mine[0]["open"]
        for r in mine:
            g.append(f'<line x1="{sx(gx):.1f}" y1="{sy(gy):.1f}" x2="{sx(r["sec"]):.1f}" y2="{sy(r["usd"]):.1f}" class="spoke"/>')
        for r in mine:
            g.append(shapes[r["test"]](sx(r["sec"]), sy(r["usd"]), "prov" if r["prov"] else ("own" if own else "host")))
        g.append(f'<circle cx="{sx(gx):.1f}" cy="{sy(gy):.1f}" r="7.5" class="{"own" if own else "host"}"/>')
        label = {"Qwen3-VL-4B, written": "Open 4B, written", "Qwen3-VL-4B, read (Glance)": "Open 4B, read (Glance)"}.get(name, name)
        left = sx(gx) > 245
        g.append(t(sx(gx) + (-12 if left else 12), sy(gy) - 3, label, "end" if left else "start", "lab own")
                 + t(sx(gx) + (-12 if left else 12), sy(gy) + 10, f"mean accuracy {acc:.2f}", "end" if left else "start", "tick"))
    g.append(t((x0 + x1) / 2, y1 + 32, "median seconds per answer (log scale): left is faster") + f'<text transform="translate(12,{(y0 + y1) / 2:.0f}) rotate(-90)" text-anchor="middle" class="tick">US dollars per 1,000 answers (log scale): down is cheaper</text>')
    key = ('<p class="legend"><svg viewBox="0 0 12 12"><circle cx="6" cy="6" r="3.2" class="host"/></svg>yes/no <svg viewBox="0 0 12 12"><rect x="3" y="3" width="6" height="6" class="host"/></svg>pick-one '
           '<svg viewBox="0 0 12 12"><polygon points="6,2.2 9.6,8.8 2.4,8.8" class="host"/></svg>rating <svg viewBox="0 0 12 12"><circle cx="6" cy="6" r="5" class="host"/></svg>centre of the three</p>')
    return key + f'<svg viewBox="0 0 360 370" role="img" aria-label="cost against speed, three tests and their centre per system">{"".join(g)}</svg>'


def option_bars(per_test):
    """Bars, each measure normalised so the BEST system is 100% (higher is better): accuracy / best accuracy, fastest / this, cheapest / this."""
    blocks = [(NAMES[k], [r for r in rows if r["test"] == k]) for k in TESTS] if per_test else [("mean of the three tests", None)]
    out = []
    for title, sub in blocks:
        stats = []
        for name in m["systems"]:
            mine = [r for r in (sub or rows) if r["name"] == name]
            stats.append({"name": name, "open": mine[0]["open"], "acc": statistics.mean(r["acc"] for r in mine),
                          "sec": math.exp(statistics.mean(math.log(r["sec"]) for r in mine)), "usd": math.exp(statistics.mean(math.log(r["usd"]) for r in mine))})
        best = {"acc": max(x["acc"] for x in stats), "sec": min(x["sec"] for x in stats), "usd": min(x["usd"] for x in stats)}
        g, y = [t(4, 12, title, "start", "head")], 22
        for x in stats:
            label = {"Qwen3-VL-4B, written": "Open 4B, written", "Qwen3-VL-4B, read (Glance)": "Open 4B, read (Glance)"}.get(x["name"], x["name"])
            g.append(t(4, y + 11, label, "start", "lab own" if x["open"] else "lab"))
            y += 16
            for key, word, score, shown in (("acc", "accuracy", x["acc"] / best["acc"], f"{x['acc']:.2f}"), ("sec", "speed", best["sec"] / x["sec"], f"{x['sec']:.2f} s"),
                                            ("usd", "cost", best["usd"] / x["usd"], money(x["usd"]) + " per 1,000")):
                width = max(1.5, score * 200)
                g.append(t(62, y + 8, word, "end") + f'<rect x="68" y="{y}" width="{width:.1f}" height="9" class="{"own" if x["open"] else "bar"}"/>' + t(68 + width + 5, y + 8, f"{score:.0%} · {shown}", "start"))
                y += 13
            y += 8
        out.append(f'<svg viewBox="0 0 360 {y}" role="img" aria-label="normalised bars, {title}">{"".join(g)}</svg>')
    return "".join(out)
