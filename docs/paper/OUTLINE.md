...
| 2.3 Reading against writing, same model | speed 2.4x to 6.1x (27c); the same accuracy on yes/no and pick-one on both photo sets, same right/wrong outcome on 95% to 99.5% of items (32b, 32c, 32d); cost per 1,000 images | `lab/GENBENCH.*`, `results/lab/cost_model.*` |
