...
| The same model WRITING its answer gives identical accuracy on yes/no and pick-one (0.931 / 0.885 on the Commons photos, both ways); reading adds speed and probabilities, not accuracy. | Supported | `results/lab/gen_accuracy.json`, entry 32b | One model, greedy decoding, no invalid outputs occurred. |
| Zero-shot ratings: the four-pass `ens4d` read (0.570) is 10 points WORSE than the same model's written answer (0.672); a one-pass read at the JSON answer position (`jsondigits`, 0.669) closes the gap, agrees with the written answer on 98.2% of items, and is ahead of Claude Opus 5 (+11.9 [7.8, 16.0]) and GPT-5.6 (+7.2 [2.7, 11.8]) and level with Gemini 3.1 Pro (+1.9 [-2.5, 6.3]) on the same 1,000 images. | Supported (five of six registered predictions; H41 missed) | `results/lab/jsondigits.json`, entries 32b, 42, 42b | Synthetic single-factor scales; one run per hosted model, no few-shot prompt for any written row; the zero-shot probabilities are NOT calibrated (ECE 0.25); not the harness default until the registered check on KADID-10k and the creative-QA rubrics (entry 43). |
| With 16 UNLABELED images of the rubric the one-pass read reaches 0.758, ahead of every hosted model's zero-shot pick (+10.8 [6.9, 14.6] over Gemini 3.1 Pro). | Supported | `results/lab/jsondigits.json`, entry 42b | Zero labels but NOT zero-shot: it needs a pool covering the rubric's range; probabilities improve (ECE 0.33 -> about 0.2 on `ens4d`) but are not calibrated. |
