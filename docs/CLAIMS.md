...
| On the lab scales a frontier model (Claude Opus 5, zero-shot) scores 0.550; the local 4B model with Glance scores 0.570 with no labels (a tie: +2.0 points [-2.1, +6.0]), 0.702 with unlabeled images (+15.2 [+10.8, +19.5]) and 0.857 with 32 labels (+30.7 [+27.3, +34.2]), same 1,000 held-out images. | Supported, with caveats | `results/lab/frontier_head_to_head.json`, `lab/NOTES.md` entry 31 | The frontier model got no examples and cannot be calibrated through a hard pick; a few-shot prompt for it was not tried. Synthetic single-factor scales. One frontier model so far. Say "matches at zero labels, ahead once it has a few images of the rubric", never "sees better than Opus". |
| Reading the answer is faster than having the same model write it: 2.4x for one yes/no, 3.5x for five mixed questions, 6.1x for 25 ratings; identical answers on yes/no. | Supported | `lab/GENBENCH.json`, entry 27c | One laptop. The written yes/no was a 7-token JSON object, not a bare word. On 25 ratings written and read answers agree on only 59% of fields and there is no ground truth for that request. |
| It works on another model family / on non-quality rubrics / against other systems' readouts / from hidden states / on fresh uncontaminated photos. | PENDING (all five) | entries 20, 22, 24, 30 | Registered with expectations that can fail. |
