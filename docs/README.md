...
| `docs/paper/RESULTS_LAB.md` | The score lab's final results (rating scales without training), generated from the lab's JSON outputs by `tools/make_results_lab.py`: selection on the calibration split, test-split table for every method, winners vs the v0 method per scale, learning curve, cross-scale transfer, reference-path check, marginal latency (with the 2026-09-20 correction), bootstrap intervals, and section 9: cold-start cost of a rating and what packing several ratings behind one image prefill saves. |
