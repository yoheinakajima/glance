...
| `docs/paper/RESULTS_ZEROSHOT.md` | THE HEADLINE (owner's framing, `lab/NOTES.md` entry 38): what the frozen open model does zero-shot as a general image decision engine (yes/no, pick-one, ratings) against frontier models zero-shot, a dual encoder and the same model writing its answer; speed and cost; self-calibration from unlabeled images as its own row; label-based results only pointed to. Generated by `tools/make_results_zeroshot.py`; pending sections fill in as data lands. |
