...
| `docs/paper/RESULTS_LAB.md` | The score lab's final results (rating scales without training), generated from the lab's JSON outputs by `tools/make_results_lab.py`: selection on the calibration split, test-split table for every method, winners vs the v0 method per scale, learning curve, cross-scale transfer, reference-path check, clean latency. |
| `docs/paper/figures/` | Paper figures (PNG + PDF) and a README with the command that regenerates them (`tools/make_lab_figures.py`). |
| `lab/NOTES.md` | The score lab's live notebook: hypotheses registered before each experiment, pilot readings, the selection rule, the entry fixing the winners before the test split was scored, final results, verdict on each hypothesis. |
