...
| `semantic` | Oxford-IIIT Pet official segmentation trimaps (`annotations/trimaps/*.png`), used to mask the same train-split photos `distort25` draws from for 7 non-distortion "creative QA" scales (framing, cutoff, off-centeredness, occlusion, tilt, text legibility, watermark), generated locally by `glance/lab/semantic.py` | https://www.robots.ox.ac.uk/~vgg/data/pets/ (`annotations.tar.gz`) | CC BY-SA 4.0 ("The dataset is available to download for commercial/research purposes under a Creative Commons Attribution-ShareAlike 4.0 International License.", dataset page) | 2026-09-20 | `annotations.tar.gz`, sha256 `52425fb6de5c424942b7626b428656fcbd798db970a937df61750c0f1d358e91` | yes |
...
| `fresh_choice`, `fresh_yesno` (opt-in) | Fresh real photos taken after the evaluated models were released (capture date >= 2026-08-15), about a dozen per class for 13 everyday classes; labels are Wikimedia Commons structured-data "depicts" statements (property P180) set by uploaders and editors, not by this project or any model | Wikimedia Commons API (`tools/fetch_fresh_depicts.py`); per-file title, page URL, author, license and dates in `glance/evals/manifests/fresh_commons_source.jsonl` | Per file, read from the API's license field; only CC0, CC BY and CC BY-SA own-work uploads are kept | 2026-09-20 | the committed source manifest (titles are stable identifiers; thumbnails are re-fetched at 1280 px) | yes: evaluation only; images live in `.cache/` and are not redistributed; attribution is kept per file |
| `inat_choice`, `inat_yesno` (opt-in) | 200 fresh real photos of organisms (20 each: bird, insect, plant, mammal, reptile, amphibian, fish, fungus, arachnid, mollusc), observed on or after 2026-08-15 and uploaded 2026-09-20; labels are the iconic taxon of iNaturalist RESEARCH-GRADE community identifications (the observer plus at least one agreeing identifier), not made by this project or any model; one photo per observer per class, 168 observers | iNaturalist API v1 (`tools/fetch_fresh_inat.py`, 10 API calls, at most 1 per second); per-photo observation URL, attribution string, observer, license, dates and taxon in `glance/evals/manifests/fresh_inat_source.jsonl` | Per PHOTO, read from the API's `license_code`; only cc0 (49), cc-by (135) and cc-by-sa (16) are kept; non-commercial licenses are excluded | 2026-09-20 | the committed source manifest (observation ids are stable; medium-size images, about 500 px, are re-fetched) | yes: evaluation only; images live in `.cache/` and are not redistributed; attribution is kept per photo |
